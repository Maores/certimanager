...
import { parseCandidateFile } from "@/app/dashboard/candidates/actions";

interface CandidateUploadStepProps {
  onParsed: (data: CandidateImportPreview) => void;
}

export function CandidateUploadStep({ onParsed }: CandidateUploadStepProps) {
  ...
  const processFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const result = await parseCandidateFile(formData);
  ...
  return (
    <div className="space-y-4">
      <div
        role="button"
        tabIndex={0}
        aria-label="העלאת קובץ Excel של מועמדים"
